...
- (void)loadImage:(NSString*)url placeHolder:(UIImage *)placeHolderImage
{
    ...
    [self sd_setImageWithURL:URL placeholderImage:placeHolderImage options:SDWebImageProgressiveDownload progress:^(NSInteger receivedSize, NSInteger expectedSize){
        if (expectedSize > 0) {
        ...
        ITTDINFO(@"is main thread %d", [NSThread isMainThread]);
